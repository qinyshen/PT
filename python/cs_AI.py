...
while True:
    ...
    commit = "CREATE TABLE IF NOT EXISTS cs_AI (No int, Date VARCHAR(50), Title VARCHAR(200), Authors VARCHAR(200), " \
    ...
    commit = "select max(No) from cs_AI;"
    cursor.execute(commit)
    num = cursor.fetchall()[0][0]

    if not num is None:
        commit = "select Address from cs_AI where No=%s;" % num
        cursor.execute(commit)
